"""Hum-to-song helpers: options, open-score trimming, prosody carrier analysis.

Two stages:
1. Score continuation - hum transcribed to ABC (SheetSage2, melody only); trailing rest-only
   lines dropped, open score placed in planner prompt so AR keeps writing it.
2. Prosody adapter - hum reduced to a carrier: pYIN pitch track drives a sine whose
   amplitude follows the low-passed hum envelope. VAE-encoded to 25 Hz latents it
   conditions the acoustic decoder (hum_nar.py).
"""
...
class HumOptions:
    ...
    melody: continue (AR continues hum's open score), hum_only (hum is the whole melody),
    or ignore (planner writes its own score; needs adapter so hum still shapes phrasing).
    adapter names a kind="hum" adapter in models/loras; influence is CFG weight on hum channel
    (1 = as trained, 0 = no hum); offset_s is where hum's carrier starts inside the song.
    """
...
def trim_open_score(abc: str) -> str:
    """Drop trailing rest-only bars / bare voice lines so score ends where hum stopped singing."""
    lines = abc.strip("\n").split("\n")
...
def analyse_hum(y: np.ndarray, *, sr: int = SAMPLE_RATE, cancelled=None) -> HumAnalysis:
    """Pitch-track a mono hum and build its prosody carrier."""
    import librosa
...
def place_condition(latents: np.ndarray, total_frames: int, offset_s: float) -> np.ndarray:
    """Zero [total_frames, 64] condition with carrier latents placed at offset_s."""
