...
class OobleckDecoder(nn.Module):
    ...
    def decode_tiled(self, z, core=1024, halo=128):
        """z [T,64] float32 → audio [1920*T-64, 2] float32, exact tile cores, no crossfade."""
        # Ensure z is an MLX array (synthesize() now returns numpy)
        z = mx.array(z)
        ...
        out_np = [np.array(t) for t in out]
        mx.clear_cache()
        return np.concatenate(out_np)
